...
check_significance <- function(var) {
  ...
  if (p_value <= significance_level) {
    ...
    emplodata <- emplodata[, !colnames(emplodata) %in% var] 
  }
}
...
cat("10 fold-validated Accuracy:", naive_bayes_model_cv$results$Accuracy, "\n")
cat("10 fold-validated Specificity:", naive_bayes_model_cv$results$Mean_Specificity, "\n")
cat("10 fold-validated Recall:", naive_bayes_model_cv$results$Mean_Recall, "\n")
